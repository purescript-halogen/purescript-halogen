...
instance functorSF :: Functor (SF i) where
  map f (SF k) = SF \i -> f <$> k i

instance functorSF1 :: Functor (SF1 i) where
  map f (SF1 o) = SF1 { result: f o.result, next: f <$> o.next }

instance applySF :: Apply (SF i) where
  apply f x = SF \i -> runSF f i <*> runSF x i

instance applySF1 :: Apply (SF1 i) where
  apply (SF1 f) (SF1 x) = SF1 { result: f.result x.result, next: f.next <*> x.next }
...
instance semigroupoidSF1 :: Semigroupoid SF1 where
  compose f g = SF1 { result: head f, next: tail f <<< tail g }
