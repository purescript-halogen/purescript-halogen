...
import Web.WebSocket.Event.EventTypes as WSET
import Web.WebSocket.Event.MessageEvent as ME
import Web.WebSocket.WebSocket as WS
...
wsProducer socket = CRA.produce \emitter -> do
  listener <- EET.eventListener \ev -> do
    for_ (readHelper ME.read ev) \msgEvent ->
